...
```bash
hdiutil convert -format UDRW -o "build/Active Agenda 0.0.0 rw.dmg" "build/Active Agenda 0.0.0.dmg"
```

Next, attach the disk image:

```bash
hdiutil attach "build/Active Agenda 0.0.0 rw.dmg"
```

This will automatically open the installer (which is really just a Finder window without the toolbars); close this for now.

Open up Finder and select "Active Agenda" from the "Devices" menu (also found at `/Volumes/Active Agenda`). From here, you can move the icons, change the folder background, etc.
...
```bash
hdiutil detach "/Volumes/Active Agenda"
hdiutil attach "build/Active Agenda 0.0.0 rw.dmg"
```
...
```bash
cp "/Volumes/Active Agenda/.DS_Store" config/dsstore
hdiutil detach "/Volumes/Active Agenda"
rm "build/Active Agenda 0.0.0 rw.dmg"
rm -rf build/source
./control.sh build
./control.sh package
```
